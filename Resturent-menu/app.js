const menu = [
  ...
  },
  {
    ...
    price: "$7",
    description:
      "Italian dessert made of savoiardi dipped in coffee, layered with a whipped mixture of eggs.",
...
function generateMenuHTML(filteredMenu) {
  ...
  filteredMenu.forEach((item) => {
    //const ShortDes = item.description.split(" ").slice(0, 3).join(" ");
    let i = 0;
    let ShortDes = "";
    while (i != 20) {
      ShortDes += item.description[i];
      i++;
    }
    ShortDes+='...';
    const menuItemHTML = `
      ...
      <div class="card" style="width: 18rem; margin:10px; ">
            ...
            <div class="card-body" >
              <h5 class="card-title" style="color: brown;">${item.title}</h5>
              ...
              <p class="card-text" style="color: blue; "><small>category: ${item.category}</p>
            </div>
          </div>
      </div>
    `;
